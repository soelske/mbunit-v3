    ...
    public class AcadCommandRunner : IAcadCommandRunner
    {
        ...
        public IAsyncResult BeginRun(AcadCommand command, IProcess process, AsyncCallback? completionCallback, object? asyncState)
        {
            ...
            if (process.HasExited)
                throw new ArgumentNullException(nameof(process), "Process has exited.");
        ...
        public void EndRun(IAsyncResult result)
        {
            ...
            if (taskResult == null)
                throw new ArgumentException("Unknown result type.", nameof(result));
        ...
        private class CommandTask
        {
            ...
            private static object GetAcadApplication(IProcess process)
            {
                ...
                while (stopwatch.Elapsed < ReadyTimeout)
                {
                    try
                    {
                        // .NET 8+ alternative to Marshal.GetActiveObject
                        var application = GetActiveObjectForNet8("AutoCAD.Application");
                        if (application != null)
                            return application;
                    }
                    catch (COMException)
            ...
            // .NET 8 compatible alternative to Marshal.GetActiveObject
            private static object? GetActiveObjectForNet8(string progId)
            {
                // Get the CLSID from the ProgID
                var clsidType = Type.GetTypeFromProgID(progId);
                if (clsidType == null)
                    return null;

                Guid clsid = clsidType.GUID;
                
                // Get the running object from the ROT (Running Object Table)
