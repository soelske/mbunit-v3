...
namespace Gallio.Runtime
{
    ...
    public class DefaultRuntime : IRuntime
    {
        ...
        private void ConfigureForDebugging()
        {
            ...
            // .NET Framework bin folder.
            if (string.IsNullOrEmpty(runtimeSetup.RuntimePath))
                runtimeSetup.RuntimePath = srcDir;

            // Only add net8.0-windows output directories. These cover both Core bridge projects
            // (bin/Debug/net8.0-windows/) and any other project that targets net8.0-windows
            // directly (e.g. Gallio.AutoCAD.Plugin/bin/v25.0/net8.0-windows/).
            // Old .NET Framework source-tree plugins are intentionally excluded: their assemblies
            // cannot load in .NET 8 and their source-dir plugin files would fail ProbeForCodeBase.
