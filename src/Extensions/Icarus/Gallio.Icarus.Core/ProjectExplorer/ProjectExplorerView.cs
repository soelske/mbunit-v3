...
namespace Gallio.Icarus.ProjectExplorer
{
    internal partial class ProjectExplorerView : UserControl
    {
        ...
        private void OpenReport()
        {
            ...
            var reportNode = (ReportNode)projectTree.SelectedNode.Tag;
            // UseShellExecute = true is required in .NET 8 (default was true in .NET Framework)
            // so the OS opens the file with its associated application instead of trying to execute it.
            Process.Start(new ProcessStartInfo(reportNode.FileName) { UseShellExecute = true });
        }
