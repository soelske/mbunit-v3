...
namespace Gallio.Runtime.Loader
{
    ...
    public class DefaultAssemblyLoader : IAssemblyLoader
    {
                ...
                if (File.Exists(assemblyPath))
                {
                    if (! IsResolvingByPartialName(assemblyName))
                    {
                        ...
                        if (assemblyName.FullName != actualAssemblyName.FullName)
                        {
                            ...
                            // .NET 8 migration: allow name+version match ignoring PublicKeyToken mismatch
                            // (unsigned .NET 8 builds vs. signed .NET Framework plugin XML bindings),
                            // and allow version-less requests (Culture=neutral, PublicKeyToken=null but
                            // no Version) to match any version on disk.
                            bool nameMatches = string.Equals(assemblyName.Name, actualAssemblyName.Name,
                                StringComparison.OrdinalIgnoreCase);
                            bool versionOk = assemblyName.Version == null
                                || assemblyName.Version == actualAssemblyName.Version;
                            if (!nameMatches || !versionOk)
                                continue;
        ...
        {
            // Simple name only — no version, culture, or PKT.
            if (name.FullName == name.Name)
                return true;

#if NET8_0_OR_GREATER
            // In .NET 8, Assembly.Load("SimpleName") fires AssemblyResolve with
            // "SimpleName, Culture=neutral, PublicKeyToken=null" (no Version).
            // Treat no-version requests as partial-name lookups.
        ...
        {
#if NET8_0_OR_GREATER
            // In .NET 8, Assembly.LoadFrom() uses the LoadFrom context which can cause
            // type-identity issues. Load into the Default context so Type.GetType() and
            // interface casts work correctly across the whole application.
