...
namespace Gallio.Runtime.Extensibility
{
    ...
    public class CachingPluginLoader : PluginLoader
    {
            ...
            try
            {
                ...
                if (Directory.Exists(cacheDirPath))
                {
                    if (File.Exists(cacheFilePath))
                    {
                        ...
                        if (oldCache != null && !HasNewPluginFiles(oldCache))
                        {
        ...
        // Detects new .plugin files in the plugin directories that are not present in the cache.
        // This handles the case where a cache was created before certain .plugin files existed
        // in already-registered directories (e.g. after building a new Core project for the first time).
        private bool HasNewPluginFiles(Cache cache)
        {
            var cachedFiles = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var info in cache.PluginInfos)
                cachedFiles.Add(info.PluginFile);

            foreach (string pluginPath in PluginPaths)
            {
                var dir = new DirectoryInfo(pluginPath);
                if (!dir.Exists) continue;
                foreach (var file in dir.GetFiles("*.plugin", SearchOption.AllDirectories))
                    if (!cachedFiles.Contains(file.FullName))
                        return true;
            }
            return false;
        }
