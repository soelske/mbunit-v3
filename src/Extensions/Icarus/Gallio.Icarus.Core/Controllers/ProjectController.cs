...
namespace Gallio.Icarus.Controllers
{
    public class ProjectController : NotifyController, IProjectController, Handles<ProjectChanged>
    {
        ...
        /// This ensures reports are saved to the same directory the FileSystemWatcher
        /// monitors, regardless of the current working directory.
        /// </summary>
        public string ReportDirectory
        {
            get
            {
                var dir = projectTreeModel.TestProject.ReportDirectory;
                if (!Path.IsPathRooted(dir) && !string.IsNullOrEmpty(projectTreeModel.FileName))
                    dir = Path.Combine(Path.GetDirectoryName(projectTreeModel.FileName), dir);
