...
namespace Gallio.Icarus
{
    ...
    public class IcarusProgram : ConsoleProgram<IcarusArguments>
    {
        ...
        {
            // Guard: null/empty means OptionsController.Load() failed before setting the value.
            if (string.IsNullOrEmpty(factoryName))
                factoryName = StandardTestRunnerFactoryNames.IsolatedProcess;
            ...
            // Fallback: if the requested factory is unavailable, try Local (no isolation required).
            if (testRunnerFactory == null && factoryName != StandardTestRunnerFactoryNames.Local)
            {
                try { testRunnerFactory = testRunnerManager.GetFactory(StandardTestRunnerFactoryNames.Local); }
                catch { }
            }
